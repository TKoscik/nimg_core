# All of Joel's processing notes in their entirety
```
...
#Need to run through all URSI/MRQID combos, determine if (and where) T1rho data is at
  #Only processing data that is URSI, then MRQID format



#Deprecated (already found numbers with exclude, 0-9 search is redundant)
#ls -F1 | grep "/" | awk -F"/" '{print $1}' | grep -Ev '[A-Z]|[a-z]' | grep -Eo '[0-9]{1,}' | grep "^.\{3\}$"
